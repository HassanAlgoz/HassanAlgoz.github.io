...
const app = new Vue({
    ...
    watch: {
        ...
        regionIndex: function(idx) {
            ...
            } else if (idx >= this.regions.length - 1) {
                this.captionCursor = this.regions.length - 1 - this.captionWindowSize;
            } else {
    ...
    async created() {
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            this.regions = this.clip.regions;
            ...
            wavesurfer.on("region-click", (region, evt) => {
                this.regionIndex = this.regions.findIndex((r, i) => r.id === region.id);
    ...
    computed: {
        ...
        // captions() {
        //     return {
        //         [this.langA]: this.regions.reduce((x, y) => x.concat(y.data[this.langA]), []),
        //         [this.langB]: this.regions.reduce((x, y) => x.concat(y.data[this.langB]), []),
        //     };
        // },
        ...
        captionWindow() {
            ...
            return this.regions.slice(Math.max(0, start), Math.min(end, this.regions.length));
        },
    },
    methods: {
        ...
        mergeLeft() {
            ...
            let idx = this.regions.findIndex(r => this.selectedRegion.id === r.id);
            if (idx === 0) return;
        ...
        mergeRight() {
            if (!this.selectedRegion) return;
            let idx = this.regions.findIndex(r => this.selectedRegion.id === r.id);
            if (idx === this.regions.length - 1) return;
        ...
        findWord(lang) {
            ...
            searchBox.regionsIndexes = this.clip.texts[lang].map((str, idx) => {
        ...
        replaceWord(lang) {
            ...
            const str = this.regions[this.regionIndex].data[lang];
            const regex = new RegExp(searchBox.word, "gmi");
            if (regex.test(str)) {
                this.regions[this.regionIndex].data[lang] = str.replace(regex, searchBox.replace);
        ...
        shiftAll() {
            ...
            for(let i = 0; i < this.regions.length; i++) {
                this.regions[i].start += this.shiftAmount;
                this.regions[i].end += this.shiftAmount;
            }
        },
        ...
        percentComplete(code) {
            ...
            let count = 0;
            for(let i = 0; i < this.regions.length; i++) {
                if (this.regions[i].data[code]) count++;
            }
            return Number.parseInt(100 * count / this.regions.length);
        },
        ...
        save () {
            ...
            //         this.isSaved = true;
            //         this.lastSaveTime = Date.now();
            //         this.regions = new Clip(this.clip).regions;
            //     })
            //     .catch((err) => {
        ...
        exportAsSRT() {
            for(const code of this.clip.languages) {
                const srt = fio.encodeSRT(this.clip.texts[code], this.clip.times);
        ...
        exportAsVTT() {
            for(const code of this.clip.languages) {
                const vtt = fio.encodeVTT(this.clip.texts[code], this.clip.times);
        ...
        exportAsSBV() {
            for(const code of this.clip.languages) {
                const sbv = fio.encodeSBV(this.clip.texts[code], this.clip.times);
        ...
        exportAsPlaintext() {
            const text = {};
            for(const code of this.clip.languages) {
                text[code] = this.regions.reduce((x, y) => x.concat(y.data[code]), []);
        ...
        play() {
            let region = this.regions[this.regionIndex];
            region.play();
        ...
        prevCaption() {
            if (!this.regions || this.regions.length === 0 || this.regionIndex === 0) return;

            if (this.regions[this.regions.length - 1].end <= wavesurfer.getCurrentTime()) {
                //
            } else {
                this.regionIndex--;
            }
        ...
        nextCaption() {
            ...
            this.regionIndex = Math.min(this.regionIndex + 1, this.regions.length - 1);
            let region = this.regions[this.regionIndex];
        ...
        remove() {
            if (!this.regions[this.regionIndex]) return;

            this.regions[this.regionIndex].remove();
            this.regions.splice(this.regionIndex, 1);
            if (!this.regions[this.regionIndex]) {
...
function loadMediaFile(file) {
    ...
    if (file.type.startsWith("audio")) {
        ...
        //     if ($video.currentTime >= wavesurfer.getDuration()) return;

        //     wavesurfer.seekAndCenter($video.currentTime / wavesurfer.getDuration());
        // });

        wavesurfer.on("seek", () => {
            if (wavesurfer.getCurrentTime() >= wavesurfer.getDuration()) return;
            app.player.currentTime(wavesurfer.getCurrentTime());
        });

        app.player.on("volumechange	", () => wavesurfer.setVolume(app.player.volume()));
        app.player.on("ratechange", () => wavesurfer.setPlaybackRate(app.player.playbackRate()));

        app.player.on("play", () => {
            if (eventSource === "wavesurfer") {
                eventSource = null;
                return;
            }
            wavesurfer.seekAndCenter(app.player.currentTime() / wavesurfer.getDuration());
            eventSource = "player";
            wavesurfer.play();
        });

        wavesurfer.on("play", () => {
            if (eventSource === "player") {
                eventSource = null;
                return;
            }
            app.player.currentTime(wavesurfer.getCurrentTime());
            eventSource = "wavesurfer";
            app.player.play();
        });
